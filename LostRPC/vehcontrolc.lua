				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if not IsPauseMenuActive() then
							...
							-- IND L
							if IsDisabledControlJustReleased(0, 189) then -- INPUT_VEH_PREV_RADIO_TRACK
								...
								count_bcast_timer = delay_bcast_timer			
							-- IND R
							elseif IsDisabledControlJustReleased(0, 190) then -- INPUT_VEH_NEXT_RADIO_TRACK
								local cstate = state_indic[veh]
								...
								count_bcast_timer = delay_bcast_timer
							-- IND H
							elseif IsControlJustReleased(0, 187) then -- INPUT_FRONTEND_CANCEL / Backspace
